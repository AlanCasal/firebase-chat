...
- 🔐 Email/password authentication with persisted auth state in React Native.
- 🧠 Global auth context drives route guarding and redirects.
- 💬 Room-based message architecture with deterministic room IDs.
- 🎨 Polished mobile UI with NativeWind, responsive sizing, and custom headers.

## Current Features

- 👤 Sign up with `username`, `email`, `password`, and profile image URL.
- 🔑 Sign in with validation and friendly auth error handling.
- 📋 Chat list screen showing other users and live last-message preview.
- 🕒 Message timestamp formatting and sender-aware message snippets.
- 💌 Real-time chat room messaging with send input, keyboard-aware layout, and autoscroll.
- 🚪 User menu with logout flow integrated in chat list header.
...
- 🖼️ [App mockup](./assets/images/app%20mockup.jpeg)
- 🖼️ [Preview 1](./assets/images/preview1.png)
- 🖼️ [Preview 2](./assets/images/preview2.png)
...
- 🧪 Add unit and E2E testing (Jest + Detox).
- 👤 Add a user profile screen.
- 🚀 Improve performance when loading chats and messages.
- 📭 Add explicit empty states for chat list.
- 📜 Improve long-list behavior and pagination handling.
